<template>
  <div class=" px-4 md:px-24 lg:px-[160px] mx-auto mt-8 flex flex-col">
    <div class="w-full mx-auto">
      ...
      <h1 class="text-3xl md:text-[34px] font-black mb-[25px]">Welcome back, Lee sinh</h1>
      <p class="font-bold text-lg md:text-[22px] mb-[15px]">Recommended for you</p>
      <div class="flex justify-start gap-4 md:gap-[20px] mb-[20px] pb-2 overflow-x-auto scrollbar-hide">

        <RecommendCard v-for="job in jobs" :key="job.title" :title="job.title" :company="job.company" />

      </div>
      <div class="flex flex-col ">
        <p class="font-bold text-lg md:text-[22px] leading-[28px] mb-[15px] ">My Applications</p>
        <div
          ...
          <div class="flex flex-col items-center relative " ref="appliedTab">
            <button @click="activeTab = 'applied'"
              :class="activeTab == 'applied' ? 'text-[#1294D4]' : 'text-black'">Applied</button>
          </div>
          <div class="flex flex-col items-center relative " ref="interviewingTab">
            <button @click="activeTab = 'interviewing'"
              :class="activeTab == 'interviewing' ? 'text-[#1294D4]' : 'text-black'">Interviewing</button>
          </div>
          <div class="flex flex-col items-center relative " ref="offerTab">
            <button @click="activeTab = 'offer'"
              :class="activeTab == 'offer' ? 'text-[#1294D4]' : 'text-black'">Offer</button>
          </div>
          <div class="absolute bottom-0 h-[3px] bg-[#1294D4] rounded-[3px]  duration-300 ease-out"
